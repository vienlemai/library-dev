/*
 * Author Lht
 * Helper to help you display flash messages, scrolling body
 * 
 */
Helper = {
    ...
    fillValidateMessages: function(res, form_id, filed_wrapper_class) {
        $.each(res, function(key, value) {
            var dom = $('#' + form_id).find('.' + filed_wrapper_class).find('#' + key);
            dom.closest('.control-group').addClass('has-error');
            $('<span class="help-inline">' + value + '</span>').insertAfter(dom);
        });
    }
};
